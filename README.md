...
To run a deterministic movement probe without keyboard input, use `test.move` with movement aliases (`w`, `a`, `s`, `d`, `forward`, `left`, `right`, `space`, `ctrl`, `shift`) plus optional millisecond timing:
