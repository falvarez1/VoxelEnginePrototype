...
- Meshlet-like terrain cluster bounds generated for each uploaded chunk, uploaded into GPU storage buffers, batched into contiguous CPU-frustum-visible ranges, and consumed by a WebGPU compute pass that writes range-level indexed-indirect draw arguments when stable Hi-Z occlusion is active. The renderer keeps direct CPU-visible range draws while Hi-Z is inactive to reduce command pressure while the camera is moving.
- GPU Hi-Z depth-pyramid generation after the main render pass, with conservative previous-frame terrain range-batch occlusion tests plus mip-count, memory, tested-batch, and culled-batch telemetry.
- Renderer-side GPU upload ring pages for terrain, far terrain, water, vegetation, and marker buffer creation, with mapped-buffer fallback telemetry.
...
- Live performance/debug overlay with frame time, worker queues, upload history, renderer upload-ring telemetry, and estimated GPU buffer memory.
...
- Near terrain now batches CPU-frustum-visible cluster ranges, uses GPU compute Hi-Z culling to zero occluded range-level indirect draws when a stable previous-frame depth pyramid is available, and uses the same contiguous ranges as direct draws while Hi-Z is inactive; it still does not build a fully GPU-generated multi-draw terrain arena.
- No shadow maps, production multi-draw meshlet pipeline, fully GPU-owned terrain draw compaction, volumetric terrain clipmaps, or full native Transvoxel LOD transition chunks yet.
...
2. Promote the current CPU-frustum range batching plus GPU Hi-Z range-indirect bridge into GPU-owned visible-cluster compaction and a larger multi-draw terrain arena.
