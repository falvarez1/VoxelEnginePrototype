...
- Native `256m` erosion tile buffer generation for height, slope, drainage, stream power, thermal erosion, hydraulic erosion, deposition, sediment load, bedrock exposure, soil depth, and vegetation retention. The thermal/hydraulic/deposition/sedimentLoad/bedrockExposure/soilDepth/vegetationRetention fields are derived from an explicit grid-based hydraulic + thermal erosion simulation: per tile, the core initialises a 17×17 height grid from `terrain_height`, runs 12 hydraulic iterations (rainfall + steepest-descent water flow + sediment capacity-based pickup/deposit + evaporation) followed by 6 thermal iterations (talus-angle-driven slumping), and writes the cumulative simulation state into the tile fields against fixed normalizers. Generator version 2.
...
- In-app developer console (`src/console.ts` + `src/console_commands.ts`) toggled with the backtick key (`` ` `` / `~`). Runs commands against the same settings sanitizer, action dispatcher, and streamer/camera/profiler/renderer/game state that URL automation uses, but interactively after the engine has initialized: `set`/`get`/`settings`, `action`/`actions`/`find`, `camera`/`tp`/`look`, `radius`/`slot`/`lod`/`near`/`far`/`water`/`vegetation`/`sky`/`markers`/`panel`, `debug`/`paint`/`brush`/`quality`, `edit`/`undo`/`redo`, `region`/`export`/`bench`, `stats`/`chunks`/`caps`/`streamer`/`probe`/`progress`, `reset`/`reset-game`/`reload`. Command history (arrow keys), tab completion, scrollback, and a `window.__stormCanyonConsole` handle for programmatic use are included.
...
`scripts/lod-selection-regression.mjs` transpiles the pure `src/terrain_lod.ts` selector and captures deterministic target counts, LOD 0/1/2+ distribution, base-cell coverage, explicit transition-face schedule samples, transition face/cell counts, per-base-cell transition worklist samples, transition sample/case summaries, transition sample coordinate/value samples, table-driven transition-prism mesh summaries, transition mesh vertex/index/cell hashes, transition-edge counts, transition-meshed chunk counts, first-request ordering, representative seam-mask requests, and the native chunk-level transition ABI's per-scene cell/vertex/triangle counts plus per-cell-vs-chunk deltas in `docs/lod-selection-baseline.json`. It is part of `npm test`, and `npm run lod:regression -- --update` is the explicit review point for intentional mixed-LOD selector, transition-schedule/worklist/case/mesh, or seam-mask changes. The transition mesh uses a deterministic tetrahedral case table over a transition prism; runtime renders an aggregate transition mesh built from cached near-chunk density samples and the native chunk-level ABI, with the runtime worker uploading sampled transition cells directly to the chunk-level WASM buffers and consuming the resulting packed mesh against a unified pack frame. The native WASM smoke test validates the per-cell ABI on all four horizontal seam sides and asserts the chunk ABI matches the per-cell loop's triangle count on a 2-cell synthetic batch.
...
- Production mixed-LOD transition chunks and Transvoxel LOD seams. The deterministic selector, LOD 1/2 outer rings, explicit transition-face schedule, per-base-cell transition worklist, transition sample/case metadata, table-driven transition-prism mesh generator, native per-cell transition ABI, native chunk-level transition ABI consumed directly by the runtime worker, TypeScript fallback mesh, plan telemetry, and regression baseline are available. The temporary coarse-edge seam-skirt geometry has been removed now that the chunk transition mesh owns the seam. Full native Transvoxel transition chunks (so transition geometry shares the same streaming/cache/persistence ownership as base chunks) are still pending.
